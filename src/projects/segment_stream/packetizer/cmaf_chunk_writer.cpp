...
#if DEBUG
		auto old_sequence_number = _sequence_number;
#endif  // DEBUG
		_sequence_number = (_start_timestamp / _ideal_duration) + 1;
		logtd("Calculated sequence number: %lld, %f = %u", _start_timestamp, _ideal_duration, _sequence_number);
#if DEBUG
		OV_ASSERT((_sequence_number - old_sequence_number) == 1,
				  "sequence number does not increase uniformly: old: %lld, new: %lld, delta: %lld",
				  old_sequence_number, _sequence_number, _sequence_number - old_sequence_number);
#endif  // DEBUG
	}
