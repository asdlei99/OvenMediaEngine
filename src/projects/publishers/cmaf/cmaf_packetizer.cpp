					 ...
					 video_track, audio_track)
{
	...
	{
		_video_chunk_writer = std::make_unique<CmafChunkWriter>(M4sMediaType::Video, 1, _ideal_duration_for_video);
	}

	if (_audio_track != nullptr)
	{
		_audio_chunk_writer = std::make_unique<CmafChunkWriter>(M4sMediaType::Audio, 2, _ideal_duration_for_audio);
	}
